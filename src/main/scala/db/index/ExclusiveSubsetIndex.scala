...
abstract class ExclusiveSubsetIndex[Q, R] extends LevelIndexParent[Q, R, scala.collection.Set[Int]] {

  override def query(context: FileContext, offset: Int, limit: Int, values: Q): R = {
    val sorted = getParameter(values).toSeq.sorted
    queryInternal(context, offset, limit, sorted, values)
  }
  ...
  override def write(context: FileContext, data: Map[Int, Q]): FileContext = {
    ...
    valuesMap.foreach { case (id, set) =>
      val seq = set.toSeq
      val combinations = (1 to set.size).flatMap(n => seq.combinations(n)).map(_.sorted)

      combinations.foreach(comb => root.insert(comb, id))
    }
